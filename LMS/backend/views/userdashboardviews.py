...
from core.custom_mixins import import ClientAdminMixin, ClientMixin, SuperAdminMixin
...
class CountCoursesStatusView(ClientMixin,APIView):
    ...
    def get(self, request):
        try:
            ...
            completed_courses_count = CourseCompletionStatusPerUser.objects.filter(enrolled_user_id=user_id, completion_status=True, in_progress_status=False,active=True).count()
            in_progress_courses_count = CourseCompletionStatusPerUser.objects.filter(enrolled_user_id=user_id, completion_status=False, in_progress_status=True,active=True).count()
            not_started_courses_count = CourseCompletionStatusPerUser.objects.filter(enrolled_user_id=user_id, completion_status=False, in_progress_status=False,active=True).count()
